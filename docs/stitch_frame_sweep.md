<!-- Results generated with the reference implementation (Wavy-Hec/CVBench bench/ — this
     ...
     chance baseline + figure scripts ported @ db18c06, added 2026-07-22. -->
...
Two consequences for the reading below. Cross-video Scene Recognition looks like a strong
category in raw accuracy but is the *weakest* margin over its own floor, because most of
its questions are binary. And Multi-video Temporal Reasoning at f64 sits only **+9.3 pts
above chance** with a ±3.3 pass std — at the top of the sweep that category is not merely
degrading, it is approaching the guessing floor.
...
Both take `--qa-json` (defaults to the `cvbench_full` subset from `configs/datasets.yaml`)
for the option counts, and `--out-dir` (defaults to `figures/frame_sweep/`, gitignored).
